...
var CONFIG_FILE_PATH = import_path.default.join(process.cwd(), ".supabase_config.json");
...
function savePersistedSupabaseConfig(url, anonKey, serviceKey) {
  try {
    const configData = {
    ...
    import_fs.default.writeFileSync(CONFIG_FILE_PATH, JSON.stringify(configData, null, 2), "utf-8");
...
function getSupabaseAdminClient(customUrl, customKey) {
...
var CENTRAL_STORE_FILE_PATH = import_path.default.join(process.cwd(), ".central_store.json");
var DEFAULT_INITIAL_CITIES = [
...
function getInitialCentralStore() {
  return {
    employees: [
      {
        id: "emp-admin",
...
var memoryCentralStore = null;
function loadCentralStore() {
  if (memoryCentralStore) return memoryCentralStore;
  try {
    if (import_fs.default.existsSync(CENTRAL_STORE_FILE_PATH)) {
      const content = import_fs.default.readFileSync(CENTRAL_STORE_FILE_PATH, "utf-8");
      ...
      if (parsed && typeof parsed === "object") {
        ...
        if (!memoryCentralStore.cities) {
          ...
          for (const initCity of DEFAULT_INITIAL_CITIES) {
            if (!memoryCentralStore.cities.some((c) => c.id === initCity.id || c.name && c.name.toLowerCase().trim() === initCity.name.toLowerCase().trim())) {
...
function saveCentralStore(store) {
  ...
  try {
    import_fs.default.writeFileSync(CENTRAL_STORE_FILE_PATH, JSON.stringify(store, null, 2), "utf-8");
  } catch (err) {
    console.warn("Could not save central store file:", err);
  }
}
function mergeArrayItems(existingList, incomingList, getKey) {
...
async function startServer() {
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "ok", service: "AutoCraft Workshop Backend", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
  });
...
Return valid JSON ONLY without markdown backticks.`;
  ...
  function generateFallbackCostEstimate(payload) {
    ...
    const mileage = vehicle.mileage || 32e3;
    ...
    const rateDiscount = isB2B ? 0.82 : 1;
    let items = [];
    if (Array.isArray(selectedJobs) && selectedJobs.length > 0) {
      items = selectedJobs.map((job, index) => {
        ...
        const adjustedPrice = Math.round(basePrice * (job.category === "PAINT" ? 1 : multiplier) * rateDiscount);
        ...
        let partNumber = `OEM-${make.slice(0, 3).toUpperCase()}-${Math.floor(1e3 + Math.random() * 9e3)}`;
        ...
        return {
          ...
          painterPayout: job.painterPayout ? Math.round(job.painterPayout * (isB2B ? 1 : 1.1)) : void 0,
          denterPayout: job.denterPayout ? Math.round(job.denterPayout * (isB2B ? 1 : 1.1)) : void 0,
    ...
    const modelInsights = [
      ...
      isLuxury ? `European electronic calipers require computerized parking brake release mode prior to pad replacement.` : `High-availability spare parts profile ensures fast same-day turnaround within ${estimatedHours} labor hours.`,
      isB2B ? `Cars24 B2B fleet rate applied with standardized pre-negotiated labor and part billing tiers.` : `Transparent baseline estimate saves customer approximately \u20B9${customerSavings.toLocaleString("en-IN")} (${savingsPercentage}%) compared to authorized showroom dealerships.`,
      mileage > 4e4 ? `At ${mileage.toLocaleString()} km, suspension bushing play and brake disc rotor thickness should be checked during the physical inspection.` : `Vehicle is in prime operational mileage window (${mileage.toLocaleString()} km).`
    ];
...
- Mileage: ${vehicle.mileage || 35e3} km
...
Return valid JSON matching the exact schema.`;
  ...
  function generateFallbackPrioritySuggestion(ctx) {
    ...
    if (isTodayOrPast && progressPct < 70) {
      ...
      recommendedActions = [
        ...
        "Flag job card as \u{1F525} URGENT for priority bay allocation."
      ];
...
Return valid JSON ONLY.`;
      ...
      if (client) {
        try {
          ...
          if (supaEmps && supaEmps.length > 0) {
            const mappedEmps = supaEmps.map((e) => {
              ...
              if (wId && supaWsData) {
                const matchedWs = supaWsData.find((w) => w.id === wId || w.name && wName && w.name.toLowerCase() === wName.toLowerCase());
                if (matchedWs) {
              ...
              if (cId && supaCitiesData && (!cName || cName.trim() === "")) {
                const matchedCity = supaCitiesData.find((c) => c.id === cId || c.name && cName && c.name.toLowerCase() === cName.toLowerCase());
                if (matchedCity) {
          ...
          if (supaCards && supaCards.length > 0) {
            const mappedCards = supaCards.map((c) => {
              ...
              if (typeof c.customer === "object" && c.customer !== null) {
                ...
                try {
                  customerObj = JSON.parse(c.customer);
                } catch {
                }
              }
              ...
              if (Array.isArray(c.tasks)) rowTasks = c.tasks;
                ...
                try {
                  rowTasks = JSON.parse(c.tasks);
                } catch {
                }
              }
              const cardIdStr = String(c.id || c.job_card_id || c.jobCardId);
              const relTasks = (supaTasks || []).filter((t) => String(t.job_card_id || t.jobCardId) === cardIdStr);
              ...
              const tasks = rawTasks.map((t, idx) => ({
                ...
                isCustomerApproved: t.is_customer_approved !== null && t.is_customer_approved !== void 0 ? Boolean(t.is_customer_approved) : t.isCustomerApproved,
              ...
              if (Array.isArray(c.qc_checklist)) qcChecklist = c.qc_checklist;
                ...
                try {
                  qcChecklist = JSON.parse(c.qc_checklist);
                } catch {
                }
              }
              let comments = [];
              ...
              let normalizedStatus = "ESTIMATE_PENDING";
              ...
              else normalizedStatus = rawStatus || "ESTIMATE_PENDING";
              return {
                id: cardIdStr,
                vehicle: {
          ...
          if (supaCities && supaCities.length > 0) {
            const mappedCities = supaCities.map((c) => ({
              id: c.id,
          ...
          if (supaWorkshops && supaWorkshops.length > 0) {
            const mappedWorkshops = supaWorkshops.map((w) => ({
              id: w.id,
          ...
          if (supaVendors && supaVendors.length > 0) {
            const mappedVendors = supaVendors.map((v) => ({
              id: v.id,
              name: v.name,
              category: v.category,
          ...
          if (supaHistory && supaHistory.length > 0) {
            const mappedHistory = supaHistory.map((h) => ({
              id: h.id,
            ...
            store.jobCardHistory = mergeArrayItems(store.jobCardHistory || [], mappedHistory, (h) => h.id);
          }
          const { data: supaCheckIns } = await client.from("vehicle_check_ins").select("*");
          if (supaCheckIns && supaCheckIns.length > 0) {
            const mappedCheckIns = supaCheckIns.map((v) => ({
              id: v.id,
          ...
          if (supaInventory && supaInventory.length > 0) {
            const mappedInventory = supaInventory.map((i) => ({
              id: i.id,
              name: i.name,
            ...
            store.inventoryItems = mergeArrayItems(store.inventoryItems || [], mappedInventory, (i) => i.id);
          }
          const { data: supaDeliveries } = await client.from("delivery_records").select("*");
          if (supaDeliveries && supaDeliveries.length > 0) {
            const mappedDeliveries = supaDeliveries.map((d) => ({
              id: d.id,
            ...
            store.deliveryRecords = mergeArrayItems(store.deliveryRecords || [], mappedDeliveries, (d) => d.id);
          }
          const { data: supaPOs } = await client.from("purchase_orders").select("*");
          if (supaPOs && supaPOs.length > 0) {
            const mappedPOs = supaPOs.map((p) => ({
              id: p.id,
            ...
            store.purchaseOrders = mergeArrayItems(store.purchaseOrders || [], mappedPOs, (p) => p.id);
          }
          const { data: supaExpenses } = await client.from("workshop_expenses").select("*");
          if (supaExpenses && supaExpenses.length > 0) {
            const mappedExpenses = supaExpenses.map((e) => ({
              id: e.id,
              title: e.title,
              category: e.category,
              amount: e.amount || 0,
            ...
            store.workshopExpenses = mergeArrayItems(store.workshopExpenses || [], mappedExpenses, (e) => e.id);
          }
          const { data: supaAttendance } = await client.from("attendance_records").select("*");
          if (supaAttendance && supaAttendance.length > 0) {
            const mappedAttendance = supaAttendance.map((a) => ({
              id: a.id,
            ...
            store.attendanceRecords = mergeArrayItems(store.attendanceRecords || [], mappedAttendance, (a) => a.id);
          }
          const { data: supaSalaries } = await client.from("salary_records").select("*");
          if (supaSalaries && supaSalaries.length > 0) {
            const mappedSalaries = supaSalaries.map((s) => ({
              id: s.id,
            ...
            store.salaryRecords = mergeArrayItems(store.salaryRecords || [], mappedSalaries, (s) => s.id);
          }
            ...
            if (Array.isArray(card.tasks) && card.tasks.length > 0) {
              for (const t of card.tasks) {
                client.from("job_tasks").upsert({
                  ...
                  is_customer_approved: t.isCustomerApproved !== void 0 ? t.isCustomerApproved : null,
  ...
  app.post("/api/central/auth/login", async (req, res) => {
    try {
      ...
      const store = loadCentralStore();
      if (["admin@fixocar.com", "admin@workshop.fixocar.com", "admin", "emp-admin"].includes(cleanId) && (!cleanPass || ["123456", "password123", "admin", "admin123"].includes(cleanPass))) {
        const empInStore = store.employees.find((e) => e.id === "emp-admin" || e.email === "admin@fixocar.com" || e.loginId === "admin");
        return res.json({
          success: true,
          user: {
            id: "emp-admin",
      ...
      if (["taifur@fixocar.com", "taifur@workshop.fixocar.com", "taifur", "emp-taifur"].includes(cleanId) && (!cleanPass || ["123456", "password123", "admin", "admin123"].includes(cleanPass))) {
        const empInStore = store.employees.find((e) => e.id === "emp-taifur" || e.email === "taifur@fixocar.com" || e.loginId === "taifur");
        return res.json({
          success: true,
          user: {
            id: "emp-taifur",
      ...
      const matchedEmp = store.employees.find(
        (e) => e.email && e.email.toLowerCase() === cleanId || e.loginId && e.loginId.toLowerCase() === cleanId || e.id && e.id.toLowerCase() === cleanId || e.email && e.email.toLowerCase().split("@")[0] === cleanId
      );
      if (matchedEmp) {
      ...
      if (client) {
        const { data: supaEmps } = await client.from("employees").select("*");
        if (supaEmps && supaEmps.length > 0) {
          const supaMatched = supaEmps.find(
            (e) => e.email && e.email.toLowerCase() === cleanId || e.login_id && e.login_id.toLowerCase() === cleanId || e.id && e.id.toLowerCase() === cleanId
          );
          if (supaMatched) {
  ...
  async function executePostSignupHook(client, authUser) {
      ...
      if (client && client.auth?.admin) {
        ...
        if (createErr) {
          if (createErr.message?.includes("already") || createErr.status === 422) {
            ...
            authUser = userList?.users?.find((u) => u.email?.toLowerCase() === cleanEmail);
          } else {
  ...
  app.post("/api/supabase/admin/diagnose-sync", async (req, res) => {
    try {
      ...
      for (const emp of employeeList) {
        ...
        const inDb = dbEmployees.some((d) => d.id === emp.id || d.email && d.email.toLowerCase() === empEmail);
        const authMatch = authUsers.find(
          (u) => u.email?.toLowerCase() === empEmail || u.user_metadata?.employee_id === emp.id || u.user_metadata?.login_id === emp.loginId
        );
        if (authMatch) {
      ...
      if (canQueryAuthUsers && authUsers.length > 0) {
        const missingInEmployeesTable = authUsers.filter((u) => {
          const uEmail = u.email?.toLowerCase().trim();
          const uEmpId = u.user_metadata?.employee_id;
          return !dbEmployees.some((d) => d.id === uEmpId || d.email && d.email.toLowerCase().trim() === uEmail);
        });
      ...
      if (action === "delete") {
        try {
          if (hasKey && client.auth?.admin && serviceKeyRole === "service_role") {
            const { data: userList } = await client.auth.admin.listUsers();
            const existing = userList?.users?.find((u) => u.email?.toLowerCase() === email || u.user_metadata?.employee_id === employee.id);
            if (existing) {
        ...
        try {
          ...
          if (listErr) {
            ...
            const existingUser = usersArray.find(
              (u) => u.email?.toLowerCase() === email || u.user_metadata?.employee_id === employee.id || u.user_metadata?.login_id === employee.loginId
            );
            if (existingUser) {
              const updatePayload = {
                user_metadata: {
  ...
  app.post("/api/supabase/auth/login", async (req, res) => {
    try {
      ...
      if ((cleanId === "admin" || cleanId === "emp-admin" || cleanId === "admin@workshop.fixocar.com") && ["123456", "password123", "admin", "admin123"].includes(cleanPass)) {
      ...
      if ((cleanId === "taifur" || cleanId === "emp-taifur" || cleanId === "taifur@workshop.fixocar.com") && ["123456", "password123", "admin", "admin123"].includes(cleanPass)) {
      ...
      if (employees && employees.length > 0) {
        const matched = employees.find(
          (e) => e.login_id && e.login_id.toLowerCase() === cleanId || e.email && e.email.toLowerCase() === cleanId || e.id && e.id.toLowerCase() === cleanId || e.name && e.name.toLowerCase() === cleanId || e.name && e.name.toLowerCase().includes(cleanId) || e.phone && e.phone.replace(/\D/g, "") === cleanId.replace(/\D/g, "") && cleanId.replace(/\D/g, "").length >= 10
        );
        if (matched) {
          ...
          let isPasswordValid = cleanPass === expectedPass || ["123456", "password123", "admin", "admin123"].includes(cleanPass);
          if (!isPasswordValid) {
      ...
      const targetEmails = /* @__PURE__ */ new Set();
      if (cleanId.includes("@")) {
      ...
      for (const candidateEmail of targetEmails) {
        try {
          ...
          if (authData?.user && !authErr) {
            const meta = authData.user.user_metadata || authData.user.raw_user_meta_data || {};
      ...
      try {
        ...
        if (userList?.users && userList.users.length > 0) {
          ...
          const matchedAuthUser = userList.users.find((u) => {
            const meta = u.user_metadata || u.raw_user_meta_data || {};
            const metaLogin = (meta.login_id || meta.loginId || "").toLowerCase();
            const metaEmp = (meta.employee_id || meta.employeeId || "").toLowerCase();
            const uEmail = (u.email || "").toLowerCase();
            const uName = (meta.name || meta.full_name || "").toLowerCase();
            return uEmail === cleanId || metaLogin === cleanId || metaEmp === cleanId || uEmail.startsWith(`${cleanId}@`) || uName && uName === cleanId;
          });
          if (matchedAuthUser) {
            ...
            if (authSuccess) {
              const meta = matchedAuthUser.user_metadata || matchedAuthUser.raw_user_meta_data || {};
              const role = meta.role || "ADMIN";
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("*", async (req, res, next) => {
      ...
      try {
        ...
        let template = fs2.readFileSync(import_path.default.resolve(process.cwd(), "index.html"), "utf-8");
